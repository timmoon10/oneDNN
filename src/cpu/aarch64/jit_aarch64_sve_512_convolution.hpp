...
struct jit_aarch64_sve_512_convolution_fwd_t : public primitive_t {
    struct pd_t : public cpu_convolution_fwd_pd_t {
        ...
        status_t init(engine_t *engine) {
            ...
            jit_aarch64_sve_512_conv_fwd_kernel::init_scratchpad(
                    scratchpad, jcp_);
...
struct jit_aarch64_sve_512_convolution_bwd_data_t : public primitive_t {
    struct pd_t : public cpu_convolution_bwd_data_pd_t {
        ...
        status_t init(engine_t *engine) {
            ...
            status_t status
                    = jit_aarch64_sve_512_conv_bwd_data_kernel_f32::init_conf(
                            jcp_, *desc(), diff_src_md_, weights_md_,
                            diff_dst_md_, dnnl_get_max_threads());
            if (status != status::success) return status;

            auto scratchpad = scratchpad_registry().registrar();
            jit_aarch64_sve_512_conv_bwd_data_kernel_f32::init_scratchpad(
                    scratchpad, jcp_);
...
struct jit_aarch64_sve_512_convolution_bwd_weights_t : public primitive_t {
    struct pd_t : public cpu_convolution_bwd_weights_pd_t {
        ...
        status_t init(engine_t *engine) {
            ...
            auto scratchpad = scratchpad_registry().registrar();
            jit_aarch64_sve_512_conv_bwd_weights_kernel_f32::init_scratchpad(
                    scratchpad, jcp_);
