...
#endif
#include "cpu/aarch64/jit_avx2_conv_kernel_f32.hpp"
...
void jit_avx2_conv_fwd_kernel_f32::width_blk_step(
    ...
    if (oc_tail) {
        push(reg_oc_blocks);
        mov(reg_oc_flag, ptr[param1_x64 + GET_OFF(oc_flag)]);
    }
    ...
    if (jcp.ndims == 5) {
        ...
        mov(reg_ki, ptr[param1_x64 + GET_OFF(kd_padding)]);
        mov(aux_reg_ker_d, ptr[param1_x64 + GET_OFF(filt)]);
        ...
        L(kd_loop);
        mov(kj, ptr[param1_x64 + GET_OFF(kh_padding)]);
    } else {
        mov(kj, reg_kh);
    }
    ...
    if (oc_tail) {
        ...
        if (jcp.with_binary && tail) {
            test(reg_ci_flag, FLAG_IC_LAST);
            je(regular_store, T_NEAR);
            if (!oc_tail) mov(reg_oc_flag, ptr[param1_x64 + GET_OFF(oc_flag)]);
            test(reg_oc_flag, FLAG_OC_LAST);
            je(regular_store, T_NEAR);
...
void jit_avx2_conv_fwd_kernel_f32::generate() {
    ...
    mov(reg_input, ptr[this->param1_x64 + GET_OFF(src)]);
    mov(reg_output, ptr[this->param1_x64 + GET_OFF(dst)]);
    mov(reg_kernel, ptr[this->param1_x64 + GET_OFF(filt)]);
    if (jcp.with_bias) mov(reg_bias, ptr[this->param1_x64 + GET_OFF(bias)]);
    mov(reg_kh, ptr[this->param1_x64 + GET_OFF(kh_padding)]);
    mov(reg_ci_flag, ptr[this->param1_x64 + GET_OFF(flags)]);
    mov(reg_oc_blocks, ptr[this->param1_x64 + GET_OFF(oc_blocks)]);

    if (is_src_layout_nxc())
        mov(reg_channel, ptr[param1_x64 + GET_OFF(reduce_work)]);
...
void jit_avx2_conv_bwd_data_kernel_f32::compute_loop(
    ...
    if (oc_tail) {
        push(reg_long_offt);
        mov(reg_reduce_work, ptr[param1_x64 + GET_OFF(reduce_work)]);
    }
    ...
    if (jcp.ndims == 5) {
        ...
        mov(reg_ki, ptr[this->param1_x64 + GET_OFF(kd_padding)]);
        cmp(reg_ki, 0);
        jle(skip_kd_loop, T_NEAR);

        mov(aux_reg_dst_d, reg_ddst);
        mov(aux_reg_ker_d, ptr[this->param1_x64 + GET_OFF(filt)]);

        L(kd_loop);
        mov(kj, ptr[this->param1_x64 + GET_OFF(kh_padding)]);
    } else {
        mov(kj, reg_kh);
    }
    ...
    if (one_of(jcp.ndims, 3, 4)) {
        ...
        L(skip_oc_loop);
        mov(reg_channel, ptr[param1_x64 + GET_OFF(channel)]);
    }
    ...
    if (ic_tail) {
        ...
        mov(reg_ci_flag, ptr[param1_x64 + GET_OFF(flags)]);
        test(reg_ci_flag, FLAG_IC_LAST);
...
void jit_avx2_conv_bwd_data_kernel_f32::generate() {
    ...
    mov(reg_dsrc, ptr[this->param1_x64 + GET_OFF(src)]);
    mov(reg_ddst, ptr[this->param1_x64 + GET_OFF(dst)]);
    mov(reg_kernel, ptr[this->param1_x64 + GET_OFF(filt)]);
    mov(reg_kh, ptr[this->param1_x64 + GET_OFF(kh_padding)]);
    mov(reg_channel, ptr[param1_x64 + GET_OFF(channel)]);
    mov(reg_channel_work, ptr[param1_x64 + GET_OFF(ch_blocks)]);
