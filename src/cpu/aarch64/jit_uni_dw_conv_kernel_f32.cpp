    ...
    for (int ch = 0; ch < ur_ch_blocks; ch++) {
        for (int ow = 0; ow < ur_w; ow++) {
            ...
            if (this->jcp.with_bias) {
                xa_->add_imm(reg_tmp_addr, reg_bias, b_off * sizeof(float),
                        reg_tmp_imm);
                xa_->ldr(zreg_acc, Xbyak_aarch64::ptr(reg_tmp_addr));
                //uni_vmovups(
                //        vmm_acc, vmmword[reg_bias + b_off * sizeof(float)]);
            } else
                xa_->fmov(zregs_acc); // zero clear

            int o_off = ch * ocb_stride + ow * ow_stride;
            if (this->jcp.with_sum) {
                xa_->add_imm(reg_tmp_addr, reg_output, o_off * sizeof(float),
                        reg_tmp_imm);
                xa_->ldr(ZReg(0), Xbyak_aarch64::ptr(reg_tmp_addr));
                xa_->fadd(zregs_acc, zregs_acc, ZRegS(0));
...
void jit_uni_dw_conv_fwd_kernel_f32<isa>::apply_filter_unrolled(
    ...
    xa_->cmp(reg_kh, 0);
    xa_->b(EQ, iter_exit_label);

    xa_->mov(iter_kh, reg_kh);
    Label kh_label;
    L(kh_label);
    {
        ...
        for (int ch = 0; ch < ur_ch_blocks; ch++) {
            for (int kw = 0; kw < jcp.kw; kw++) {
                ...
                for (int ow = ow_start; ow < ow_end; ow++) {
                    ...
                    xa_->add_imm(reg_tmp_addr, aux_reg_input,
                            inp_off * jcp.typesize_in, reg_tmp_imm);
                    xa_->ldr(zreg_src, Xbyak_aarch64::ptr(reg_tmp_addr));
        ...
        if (jcp.is_fused_conv) {
            ...
            xa_->add_imm(aux_reg_input, aux_reg_input,
                    ih_stride * dilate_h * sizeof(float), reg_tmp_imm);
        }

        xa_->sub(iter_kh, iter_kh, 1); //dec(iter_kh);
        xa_->cmp(iter_kh, 0);
        xa_->b(GT, kh_label);
    }
...
void jit_uni_dw_conv_fwd_kernel_f32<isa>::store_dst(
    ...
    for (int ch = 0; ch < ur_ch_blocks; ch++) {
        for (int ow = 0; ow < ur_w; ow++) {
            ...
            xa_->add_imm(reg_tmp_addr, reg_output, o_off * sizeof(float),
                    reg_tmp_imm);
            xa_->str(zreg_dst, Xbyak_aarch64::ptr(reg_tmp_addr));
        }
    }
}
...
void jit_uni_dw_conv_fwd_kernel_f32<isa>::compute_loop(
    ...
    auto compute = [&](int ur_ch_blocks) {
        if (jcp.is_fused_conv) {
            xa_->mov(aux_reg_input_buffer_ptr, reg_input_buffer_ptr);
        } else {
    ...
    if (ch_loop) {
        ...
        xa_->mov(aux_reg_ch_blocks, reg_ch_blocks);
        xa_->mov(reg_kernel_stack, reg_kernel);
        xa_->mov(reg_input_stack, reg_input);
        xa_->mov(reg_output_stack, reg_output);
        if (jcp.with_bias) xa_->mov(reg_bias_stack, reg_bias);

        if (ch_tail) {
            xa_->cmp(aux_reg_ch_blocks, jcp.nb_ch_blocking);
            xa_->b(LT, ch_tail_label);
        }

        L(ch_loop_label);
        {
            compute(jcp.nb_ch_blocking);
            xa_->add_imm(reg_kernel, reg_kernel, wei_ch_stride, reg_tmp_imm);
            xa_->add_imm(reg_input, reg_input, inp_ch_stride, reg_tmp_imm);
        ...
        if (ch_tail) {
            L(ch_tail_label);
            xa_->cmp(aux_reg_ch_blocks, 0);
            xa_->b(LE, skip_ch_tail_label);
            compute(ch_tail);
            L(skip_ch_tail_label);
        }

        if (jcp.with_bias) xa_->mov(reg_bias, reg_bias_stack);
        xa_->mov(reg_output, reg_output_stack);
        xa_->mov(reg_input, reg_input_stack);
        xa_->mov(reg_kernel, reg_kernel_stack);
...
void jit_uni_dw_conv_fwd_kernel_f32<isa>::ow_loop(int ur_ch_blocks) {
    ...
    if (ow == ur_w) {
        ...
        if (n_oi == 0) {
            ...
            xa_->add_imm(reg_input, reg_input, inp_shift_pad, reg_tmp_imm);
...
void jit_uni_dw_conv_fwd_kernel_f32<isa>::generate() {
    ...
    if (jcp.is_fused_conv) {
        ...
        Traverse the data as
            xa_->mov(reg_data, ptr[reg_input_buffer_ptr])
            ... process row0 ...
            xa_->add(reg_input_buffer_ptr, sizeof(void*))
            xa_->mov(reg_data, ptr[reg_input_buffer_ptr])
            ... process row1 ...
            xa_->add(reg_input_buffer_ptr, sizeof(void*))
            xa_->mov(reg_data, ptr[reg_input_buffer_ptr])
            ... process row2 ...
        */
        xa_->mov(reg_iw_offset, 0);
    } else {
        xa_->ldr(reg_input, Xbyak_aarch64::ptr(abi_param1, GET_OFF(src)));
    }
    ...
    if (is_src_layout_nxc()) {
        ...
        xa_->cmp(reg_ch_blocks, jcp.nb_ch_blocking);
        xa_->b(NE, ch_blocks_tail ? ch_blocks_tail_label : exit_label);
        ...
        if (ch_blocks_tail) {
            ...
            xa_->cmp(reg_ch_blocks, ch_blocks_tail);
            xa_->b(NE, exit_label);
...
inline void jit_uni_dw_conv_bwd_data_kernel_f32<isa>::apply_filter(
    ...
    L(kh_label);
    { // KH loop
        xa_->mov(aux1_reg_ddst, aux_reg_ddst);
        xa_->mov(aux1_reg_kernel, aux_reg_kernel);

        xa_->mov(iter_kw, reg_kw);
        Label kw_label;
        L(kw_label);
        { // KW loop
            ...
            xa_->add_imm(aux1_reg_kernel, aux1_reg_kernel,
                    ch_blk * stride_w * sizeof(float), reg_tmp_imm);
            xa_->sub_imm(aux1_reg_ddst, aux1_reg_ddst,
                    ch_blk * (jcp.dilate_w + 1) * sizeof(float), reg_tmp_imm);
            xa_->sub_imm(iter_kw, iter_kw, stride_w, reg_tmp_imm);
            xa_->cmp(iter_kw, 0);
            xa_->b(GT, kw_label);
        }
        ...
        xa_->sub_imm(aux_reg_ddst, aux_reg_ddst,
                ow * ch_blk * (jcp.dilate_h + 1) * sizeof(float), reg_tmp_imm);
        xa_->sub_imm(iter_kh, iter_kh, stride_h, reg_tmp_imm);
        xa_->cmp(iter_kh, 0);
        xa_->b(GT, kh_label);
    }
...
inline void jit_uni_dw_conv_bwd_data_kernel_f32<isa>::store_dsrc(
    ...
    for (int ch = 0; ch < ur_ch_blocks; ch++) {
        for (int w = 0; w < ur_str_w; w++) {
            ...
            xa_->add_imm(reg_tmp_addr, reg_dsrc, dsrc_off * sizeof(float),
                    reg_tmp_imm);
            xa_->str(zreg_acc, Xbyak_aarch64::ptr(reg_tmp_addr));
        }
    }
}
...
inline void jit_uni_dw_conv_bwd_data_kernel_f32<isa>::loop_body(
    ...
    L(unrolled_w_label);
    {
        ...
        xa_->cmp(reg_ur_str_w, ur_w);
        xa_->b(LT, tail_w_label);

        xa_->mov(aux_reg_ddst, reg_ddst);
        xa_->mov(aux_reg_kernel, reg_kernel);
        ...
        xa_->add_imm(reg_dsrc, reg_dsrc,
                sizeof(float) * ur_w * jcp.ch_block * jcp.stride_w,
                reg_tmp_imm);
        xa_->add_imm(reg_ddst, reg_ddst, sizeof(float) * ur_w * jcp.ch_block,
                reg_tmp_imm);

        xa_->sub_imm(reg_ur_str_w, reg_ur_str_w, ur_w, reg_tmp_imm);
        xa_->b(unrolled_w_label);
    }
...
void jit_uni_dw_conv_bwd_data_kernel_f32<isa>::generate() {
    ...
    xa_->ldr(reg_kernel, Xbyak_aarch64::ptr(abi_param1, GET_OFF(filt)));
    xa_->ldr(reg_kh, Xbyak_aarch64::ptr(abi_param1, GET_OFF(kh_padding)));
    xa_->ldr(reg_kw, Xbyak_aarch64::ptr(abi_param1, GET_OFF(kw_padding)));
    xa_->ldr(reg_ch_blocks, Xbyak_aarch64::ptr(abi_param1, GET_OFF(ch_blocks)));
    xa_->ldr(reg_ur_str_w, Xbyak_aarch64::ptr(abi_param1, GET_OFF(ur_str_w)));
...
inline void jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::load_filter() {
    for (int i = 0; i < jcp.kw; ++i) {
        ...
        xa_->add_imm(reg_tmp_addr, reg_tmp_filter, off_filter * sizeof(float),
                reg_tmp_imm);
        if (simd_w == 16) {
            ...
            ZRegS zregs_acc = get_acc_reg_s(i);
            ld1w(zregs_acc, reg_p_all_ones, Xbyak_aarch64::ptr(reg_tmp_addr));
        } else {
...
inline void jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::zero_bias() {
    ...
    xa_->fmov(zregs_bias); // zero clear
}
template <cpu_isa_t isa>
inline void jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::load_bias() {
    if (simd_w == 16) {
        ZReg zreg_bias = get_bias_reg(0);
        xa_->ldr(zreg_bias, Xbyak_aarch64::ptr(reg_bias_baddr));
    } else if (simd_w == 8) {
        ZRegS zregs_bias = get_bias_reg_s(0);
        ld1w(zregs_bias, reg_p_all_ones, Xbyak_aarch64::ptr(reg_bias_baddr));
    } else {
        assert(!"Unsupport: simd_w != 16, 8");
    }
}
...
inline void jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::compute_ow_step_unroll(
    ...
    for (int i_ur = 0; i_ur < unroll_w; ++i_ur) {
        ...
        xa_->add_imm(reg_tmp_addr, reg_tmp_output, off_output * sizeof(float),
                reg_tmp_imm);
        if (simd_w == 16) {
        ...
        for (int i_kw = 0; i_kw < jcp.kw; ++i_kw) {
            ...
            xa_->fmla(zregs_acc, reg_p_all_ones, zregs_aux, zregs_output);
        }
    }
}
...
jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::compute_bias_step_unroll(
    ...
    for (int i = 0; i < unroll_w; ++i) {
        ...
        xa_->add_imm(reg_tmp_addr, reg_tmp_output, off_output * sizeof(float),
                reg_tmp_imm);
        if (simd_w == 16) {
            xa_->ldr(ZReg(31), Xbyak_aarch64::ptr(reg_tmp_addr));
        } else if (simd_w == 8) {
            ld1w(ZRegS(31), reg_p_all_ones, Xbyak_aarch64::ptr(reg_tmp_addr));
        } else {
            assert(!"Unsupport: simd_w != 16, 8");
        }
        xa_->fadd(zregs_bias, zregs_bias, ZRegS(31));
    }
}
...
inline void jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::store_filter() {
    for (int i = 0; i < jcp.kw; ++i) {
        ...
        if (simd_w == 16) {
            ...
            ZRegS zregs_acc = get_acc_reg_s(i);
            st1w(zregs_acc, reg_p_all_ones, Xbyak_aarch64::ptr(reg_tmp_addr));
        } else {
...
inline void jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::store_bias() {
    if (simd_w == 16) {
        ZReg zreg_bias = get_bias_reg(0);
        xa_->str(zreg_bias, Xbyak_aarch64::ptr(reg_bias_baddr));
    } else if (simd_w == 8) {
        ZRegS zregs_bias = get_bias_reg_s(0);
        st1w(zregs_bias, reg_p_all_ones, Xbyak_aarch64::ptr(reg_bias_baddr));
    } else {
        assert(!"Unsupported: simd_w != 16, 8");
    }
}
...
inline void jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::compute_bias_loop(
    ...
    xa_->ldr(reg_oh_worksize,
            Xbyak_aarch64::ptr(abi_param1,
                    static_cast<int32_t>(
                            offsetof(jit_dw_conv_call_s, oh_count))));

    xa_->mov(reg_tmp_output, reg_output_baddr);
    L(oh_label);
    {

        xa_->mov_imm(reg_iter_ow_blk, unroll_w_trips);
        L(ow_blk_label);
        {

            compute_bias_step_unroll(unroll_w);
            xa_->add_imm(reg_tmp_output, reg_tmp_output,
                    unroll_w * ch_offset * sizeof(float), reg_tmp_imm);

            xa_->sub(reg_iter_ow_blk, reg_iter_ow_blk,
                    1); //dec(reg_iter_ow_blk);
            xa_->cmp(reg_iter_ow_blk, 0);
            xa_->b(GT, ow_blk_label);
        }
        ...
        xa_->add(reg_oh, reg_oh, 1); //inc(reg_oh);
        xa_->cmp(reg_oh, reg_oh_worksize);
        xa_->b(LT, oh_label);
    }
}
...
inline void jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::compute_zero_filter() {
    ...
    xa_->ldr(reg_exec_flags,
            Xbyak_aarch64::ptr(abi_param1,
                    static_cast<int32_t>(
                            offsetof(jit_dw_conv_call_s, exec_flags))));
    xa_->and_(reg_exec_flags, reg_exec_flags, FLAG_ZERO_FILTER);
    tst(reg_exec_flags, reg_exec_flags);
    xa_->b(EQ, skip_zeroing_label);

    zero_filter();

    xa_->mov(reg_tmp_filter, reg_filter_baddr);
    xa_->mov_imm(reg_kh, jcp.kh);
    L(kh_loop_label);
    {
        store_filter();

        xa_->add_imm(reg_tmp_filter, reg_tmp_filter,
                jcp.kw * ch_offset * sizeof(float), reg_tmp_imm);
        xa_->sub(reg_kh, reg_kh, 1); //dec(reg_kh);
        xa_->cmp(reg_kh, 0);
        xa_->b(GT, kh_loop_label);
    }
...
inline void jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::compute_h_step(
    ...
    xa_->cmp(reg_kh_count, 0);
    xa_->b(EQ, skip_loop_label);

    xa_->mov(reg_kh, reg_kh_count);
    L(kh_loop_label);
    {
        load_filter();
        ...
        xa_->add_imm(reg_tmp_filter, reg_tmp_filter,
                jcp.kw * ch_offset * sizeof(float), reg_tmp_imm);
        xa_->add_imm(reg_tmp_input, reg_tmp_input,
    ...
    L(kh_comeback_label);
    {
        xa_->sub_imm(reg_tmp_input, reg_tmp_input,
                jcp.iw * ch_offset * sizeof(float), reg_tmp_imm);
        xa_->sub_imm(reg_tmp_filter, reg_tmp_filter,
                jcp.kw * ch_offset * sizeof(float), reg_tmp_imm);
        xa_->sub(reg_kh, reg_kh, 1); //dec(reg_kh);
        xa_->cmp(reg_kh, 0);
        xa_->b(GT, kh_comeback_label);
    }
...
inline void jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::compute_h_loop(
    ...
    xa_->ldr(reg_oh_worksize,
            Xbyak_aarch64::ptr(abi_param1,
                    static_cast<int32_t>(
                            offsetof(jit_dw_conv_call_s, oh_count))));
    xa_->ldr(reg_kh_count,
            Xbyak_aarch64::ptr(abi_param1,
                    static_cast<int32_t>(
                            offsetof(jit_dw_conv_call_s, kh_count))));

    xa_->mov(reg_tmp_output, reg_output_baddr);
    xa_->mov(reg_tmp_input, reg_input_baddr);
    xa_->mov(reg_tmp_filter, reg_filter_baddr);
    ...
    L(h_loop_label);
    {
        ...
        if (jcp.t_pad > 0) {
            ...
            xa_->cmp(reg_oh, jcp.t_pad);
            xa_->b(GT, skip_tpad_label);
            ...
            xa_->b(tpad_loop_label);
        }

        L(skip_tpad_label);

        xa_->cmp(reg_oh, io_overlap);
        xa_->b(LT, skip_bpad_label);
        xa_->sub_imm(reg_kh_count, reg_kh_count, b_overlap_off, reg_tmp_imm);
...
jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::compute_ow_block_unroll() {
    ...
    if (jcp.with_bias) {
        ...
        xa_->and_(reg_exec_flags, reg_exec_flags, FLAG_ZERO_BIAS);
        tst(reg_exec_flags, reg_exec_flags);
        xa_->b(NE, skip_load_bias); //jne(skip_load_bias);
    ...
    compute_zero_filter();
    xa_->ldr(reg_kh_offset,
            Xbyak_aarch64::ptr(abi_param1,
                    static_cast<int32_t>(
                            offsetof(jit_dw_conv_call_s, filter_pad_off))));
    xa_->add(reg_filter_baddr, reg_filter_baddr, reg_kh_offset);

    /* compute left padded block */
    if (l_pad && do_unroll_w) {
        compute_h_loop(unroll_w, l_pad, 0, 0);
        xa_->add_imm(reg_output_baddr, reg_output_baddr,
                unroll_w * ch_offset * sizeof(float), reg_tmp_imm);
        xa_->add_imm(reg_input_baddr, reg_input_baddr,
    ...
    if (do_ow_blk_loop) {
        xa_->sub(reg_iter_ow_blk, reg_iter_ow_blk, 1);
        xa_->cmp(reg_iter_ow_blk, 0);
        xa_->b(GT, ow_blk_label);
    }
...
void jit_uni_dw_conv_bwd_weights_kernel_f32<isa>::generate() {
    ...
    if (simd_w == 16) {
        ...
        xa_->ptrue(reg_p_all_ones.b, VL32);
    } else {
        assert(!"Unsupport: simd_w != 16, 8");
    }
    xa_->ldr(reg_input_baddr,
            Xbyak_aarch64::ptr(abi_param1,
                    static_cast<int32_t>(offsetof(jit_dw_conv_call_s, input))));
    xa_->ldr(reg_output_baddr,
            Xbyak_aarch64::ptr(abi_param1,
                    static_cast<int32_t>(
                            offsetof(jit_dw_conv_call_s, output))));
    xa_->ldr(reg_filter_baddr,
            Xbyak_aarch64::ptr(abi_param1,
                    static_cast<int32_t>(
