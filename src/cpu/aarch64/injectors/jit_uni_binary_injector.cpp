...
static void push_treg(jit_generator *host, const TReg &treg) {
    host->sub_imm(host->X_SP, host->X_SP,
            injector_utils::vmm_size_t<TReg>::bytes, host->X_TMP_0);
    host->str(treg, Xbyak_aarch64::ptr(host->X_SP));
}

template <>
static void push_treg<Xbyak_aarch64::VReg>(
        jit_generator *host, const Xbyak_aarch64::VReg &treg) {
    host->sub_imm(host->X_SP, host->X_SP,
            injector_utils::vmm_size_t<Xbyak_aarch64::VReg>::bytes,
            host->X_TMP_0);
    host->str(
            Xbyak_aarch64::QReg(treg.getIdx()), Xbyak_aarch64::ptr(host->X_SP));
}
...
template <>
static void pop_treg<Xbyak_aarch64::VReg>(
        jit_generator *host, const Xbyak_aarch64::VReg &treg) {
    host->ldr(
            Xbyak_aarch64::QReg(treg.getIdx()), Xbyak_aarch64::ptr(host->X_SP));
    host->add_imm(host->X_SP, host->X_SP,
...
static void restore_stack(jit_generator *host, const TReg &treg) {
    host->add(host->X_SP, injector_utils::vmm_size_t<TReg>::bytes);
}
...
Xbyak_aarch64::AdrNoOfs jit_uni_binary_injector_t<isa>::prepare_rhs_arg_addr(
    ...
    CG::add_imm(Xbyak_aarch64::XReg(28), param1_, abi_param_offset,
            Xbyak_aarch64::XReg(23));
    CG::ldr(rhs_addr_reg, ptr(Xbyak_aarch64::XReg(28)));
    CG::add_imm(Xbyak_aarch64::XReg(28), rhs_addr_reg,
            rhs_arg_idx * rhs_arg_ptr_size, Xbyak_aarch64::XReg(23));
    CG::ldr(rhs_addr_reg, ptr(Xbyak_aarch64::XReg(28)));
...
void jit_uni_binary_injector_t<isa>::append_offset_from_operand(
        const std::map<int, Xbyak_aarch64::XReg> &vmm_idx_to_elem_operand_off,
...
void jit_uni_binary_injector_t<isa>::append_offset_under_mem_addr(
    ...
    if (it_off_addr != vmm_idx_to_elem_addr_off.end()) {
        if (elem_size_bytes == 1) {
            ...
            const int shift_val = std::log2(elem_size_bytes);
            CG::ldr(tmp_reg, Xbyak_aarch64::ptr(it_off_addr->second.getXn()));
            CG::lsl(tmp_reg, tmp_reg, shift_val);
            CG::add(addr_reg, addr_reg, tmp_reg);
        }
    }
...
void jit_uni_binary_injector_t<isa>::execute_broadcast_s8u8_no_tail(
    ...
    const TReg treg(tmp_vmm.getIdx());
    switch (data_type) {
        case data_type::s8:
...
void jit_uni_binary_injector_t<isa>::load_rhs_tail(
    ...
    if (vlen == 64) {
        CG::eor(Xbyak_aarch64::ZReg(IDX(tmp_vmm)).d,
                Xbyak_aarch64::ZReg(IDX(tmp_vmm)).d,
                Xbyak_aarch64::ZReg(IDX(tmp_vmm)).d);
    } else if (vlen == 32) {
        CG::ptrue(Xbyak_aarch64::PRegB(1), Xbyak_aarch64::VL32);
        CG::eor(Xbyak_aarch64::ZReg(IDX(tmp_vmm)).d,
                Xbyak_aarch64::ZReg(IDX(tmp_vmm)).d,
                Xbyak_aarch64::ZReg(IDX(tmp_vmm)).d);
        CG::mov(Xbyak_aarch64::ZReg(IDX(tmp_vmm)).s,
                Xbyak_aarch64::PReg(1) / Xbyak_aarch64::T_m, 0);
    } else if (vlen == 16) {
        CG::eor(Xbyak_aarch64::VReg16B(IDX(tmp_vmm)),
                Xbyak_aarch64::VReg16B(IDX(tmp_vmm)),
                Xbyak_aarch64::VReg16B(IDX(tmp_vmm)));
    } else {
        assert(!"unreachable");
    }
    switch (data_type) {
        ...
        case data_type::s32:
            if (vlen == 64) {
                CG::pfalse(Xbyak_aarch64::PRegB(9));
                CG::zip1(Xbyak_aarch64::PRegB(1),
                        Xbyak_aarch64::PRegB(IDX(tail_opmask)),
                        Xbyak_aarch64::PRegB(9));
                CG::zip1(Xbyak_aarch64::PRegH(1), Xbyak_aarch64::PRegH(1),
                        Xbyak_aarch64::PRegH(9));
                CG::ld1w(Xbyak_aarch64::ZRegS(IDX(tmp_vmm)),
                        Xbyak_aarch64::PReg(1) / Xbyak_aarch64::T_z,
                        Xbyak_aarch64::ptr(rhs_addr.getXn()));
            } else if (vlen == 32) {
                CG::sub(Xbyak_aarch64::XReg(22), Xbyak_aarch64::XReg(22), 8);
                CG::str(Xbyak_aarch64::PReg(7),
                        Xbyak_aarch64::ptr(Xbyak_aarch64::XReg(22)));
                CG::ptrue(Xbyak_aarch64::PRegB(7));
                CG::ptrue(Xbyak_aarch64::PRegB(14), Xbyak_aarch64::VL32);
                CG::bic(Xbyak_aarch64::PRegB(7),
                        Xbyak_aarch64::PReg(7) / Xbyak_aarch64::T_z,
                        Xbyak_aarch64::PRegB(IDX(tail_opmask)),
                        Xbyak_aarch64::PRegB(14));
                CG::ld1w(Xbyak_aarch64::ZRegS(IDX(tmp_vmm)),
                        Xbyak_aarch64::PReg(7) / Xbyak_aarch64::T_z,
                        Xbyak_aarch64::ptr(rhs_addr.getXn()));
                CG::ldr(Xbyak_aarch64::PReg(7),
                        Xbyak_aarch64::ptr(Xbyak_aarch64::XReg(22)));
                CG::add(Xbyak_aarch64::XReg(22), Xbyak_aarch64::XReg(22), 8);
            } else if (vlen == 16) {
                CG::sub(Xbyak_aarch64::XReg(22), Xbyak_aarch64::XReg(22), 8);
                CG::str(Xbyak_aarch64::PReg(7),
                        Xbyak_aarch64::ptr(Xbyak_aarch64::XReg(22)));
                CG::ptrue(Xbyak_aarch64::PRegB(7));
                CG::ptrue(Xbyak_aarch64::PRegB(13), Xbyak_aarch64::VL16);
                CG::bic(Xbyak_aarch64::PRegB(7),
                        Xbyak_aarch64::PReg(7) / Xbyak_aarch64::T_z,
                        Xbyak_aarch64::PRegB(IDX(tail_opmask)),
                        Xbyak_aarch64::PRegB(13));
                CG::ld1w(Xbyak_aarch64::ZRegS(IDX(tmp_vmm)),
                        Xbyak_aarch64::PReg(7) / Xbyak_aarch64::T_z,
                        Xbyak_aarch64::ptr(rhs_addr.getXn()));
                CG::ldr(Xbyak_aarch64::PReg(7),
...
void jit_uni_binary_injector_t<isa>::execute_binary(alg_kind_t binary_alg,
    ...
    switch (binary_alg) {
        case alg_kind::binary_add:
            CG::fadd(Xbyak_aarch64::ZReg(IDX(dst)).s,
...
void jit_uni_binary_injector_t<isa>::execute_binary(alg_kind_t binary_alg,
    ...
    switch (binary_alg) {
        case alg_kind::binary_add:
            CG::ptrue(Xbyak_aarch64::PReg(1).b);
            CG::sub_imm(Xbyak_aarch64::XReg(22), Xbyak_aarch64::XReg(22), 64,
                    Xbyak_aarch64::XReg(23));
            CG::st1w(Xbyak_aarch64::ZRegS(1), Xbyak_aarch64::PReg(1),
                    Xbyak_aarch64::ptr(Xbyak_aarch64::XReg(22)));
            CG::ldr(Xbyak_aarch64::ZReg(1), Xbyak_aarch64::ptr(rhs.getXn()));
            CG::fadd(Xbyak_aarch64::ZReg(IDX(dst)).s,
                    Xbyak_aarch64::ZReg(IDX(lhs)).s, Xbyak_aarch64::ZReg(1).s);
            CG::ld1w(Xbyak_aarch64::ZRegS(1), Xbyak_aarch64::PReg(1),
                    Xbyak_aarch64::ptr(Xbyak_aarch64::XReg(22)));
            CG::add_imm(Xbyak_aarch64::XReg(22), Xbyak_aarch64::XReg(22), 64,
                    Xbyak_aarch64::XReg(23));
            break;
        case alg_kind::binary_mul: /*unsupported*/ break;
        case alg_kind::binary_max: /*unsupported*/ break;
