...
inline void jit_uni_pool_kernel<isa>::push_vmm_val(const int idx) {
    ...
    sub_imm(X_SP, X_SP, val_to_store.getBit() / 8, X_TMP_0);
    str(val_to_store, Xbyak_aarch64::ptr(X_SP));
}
...
inline void jit_uni_pool_kernel<isa>::pop_vmm_val(const int idx) {
    ...
    ldr(val_to_load, Xbyak_aarch64::ptr(X_SP));
    add_imm(X_SP, X_SP, val_to_load.getBit(), X_TMP_0);
}
...
inline void jit_uni_pool_kernel<isa>::load(const int idx, const xreg_t &reg_ptr,
    ...
    if (is_c_tail_proccessing && !jpp.is_c_padded) {
        ...
        ld1w(ZRegS(idx), p_tmp0 / Xbyak_aarch64::T_z,
...
inline void jit_uni_pool_kernel<isa>::store(const int idx,
    ...
    if (is_c_tail_proccessing && !jpp.is_c_padded) {
        ...
        zip1(p_tmp0.b, k_c_tail_mask.b, p_all_zero.b);
        zip1(p_tmp0.h, p_tmp0.h, p_all_zero.h);
        st1w(ZRegS(idx), p_tmp0, Xbyak_aarch64::ptr(X_DEFAULT_ADDR));
    } else {
        add_imm(X_DEFAULT_ADDR, reg_ptr, offset, X_TMP_0);
        st1w(ZRegS(idx), p_lsb, Xbyak_aarch64::ptr(X_DEFAULT_ADDR));
    }
}
...
inline void jit_uni_pool_kernel<isa>::maybe_recalculate_divisor(
    ...
    if (jpp.alg == pooling_avg_exclude_padding) {
        ...
        if (non_zero_kw != prev_kw) {
            ...
            ptrue(p_tmp0.d, VL2);
            xa_->mov(ZRegD(xmm_tmp.getIdx()), p_tmp0 / T_m, 0);
            ptrue(p_tmp0.d, VL1);
            xa_->mov(ZRegD(xmm_tmp.getIdx()), p_tmp0 / T_m, tmp_gpr);

            dup(vmm_tmp.s, ZRegS(xmm_tmp.getIdx())[0]);

            xa_->fmul(vmm_tmp.s, vmm_tmp.s, vmm_ker_area_h);
            prev_kw = non_zero_kw;
        }
    }
}
...
inline void jit_uni_pool_kernel<isa>::avg_step(int ur_w, int ur_bc, int pad_l,
    ...
    for (int jj = 0; jj < ur_w; jj++) {
        ...
        for (int bci = 0; bci < ur_bc; bci++) {
            ...
            if (jpp.is_backward) {
                ...
                xa_->fdiv(accvr.s, p_512, vmm_tmp.s);
            } else {
    ...
    if (jpp.simple_alg && jpp.ndims == 5) {
        ...
        ldr(ki, Xbyak_aarch64::ptr(X_DEFAULT_ADDR));
        L(kd_label);
        ...
        for (int ki = 0; ki < kw; ki++) {
            ...
            for (int bci = 0; bci < ur_bc; bci++) {
                ...
                if (jpp.is_backward) {
                    ...
                    xa_->fadd(inpvr.s, inpvr.s, accvr);
                    ...
                    if (is_tail_processing(bci)) {
                        load(vmm_tmp_1.getIdx(), aux_xreg_input, input_offset,
                                is_tail_processing(bci));
                        xa_->fadd(accvr, accvr, vmm_tmp_1);
                    } else {
                        xa_->add_imm(X_DEFAULT_ADDR, aux_reg_input,
                                input_offset, X_TMP_0);
                        ldr(z_tmp0, Xbyak_aarch64::ptr(X_DEFAULT_ADDR));
                        xa_->fadd(accvr, accvr, z_tmp0.s);
                    }
                }
            }
        }
...
inline void jit_uni_pool_kernel<isa>::max_step_fwd(int ur_w, int ur_bc,
    ...
    if (jpp.is_training) {
        ...
        xa_->mov(ZRegD(xmm_tmp.getIdx()), p_tmp0 / T_m, 0);
        ptrue(p_tmp0.d, VL1);
        xa_->mov(ZRegD(xmm_tmp.getIdx()), p_tmp0 / T_m, reg_k_shift);
    ...
    L(kh_label);
    {
        for (int ki = 0; ki < kw; ki++) {
            ...
            for (int bci = 0; bci < ur_bc; bci++) {
                ...
                fcmlt(k_store_mask.s, p_512 / Xbyak_aarch64::T_z, accvr, inpvr);
                sel(accvr, k_store_mask / T_m, inpvr, accvr);
                if (jpp.is_training) {
                    sel(indvr, k_store_mask / T_m, vmm_k_offset, indvr);
                }
            }
            if (jpp.is_training) xa_->add(vmm_k_offset, vmm_k_offset, vmm_one);
        }
    ...
    if (jpp.ndims == 5) {
        ...
        if (jpp.is_training) {
            add_imm(X_DEFAULT_ADDR, reg_param, GET_OFF(kd_padding_shift),
                    X_TMP_0);
            ldr(tmp_gpr, Xbyak_aarch64::ptr(X_DEFAULT_ADDR));
            ...
            dup(vmm_tmp.s, ZRegS(xmm_tmp.getIdx())[0]);
            xa_->add(vmm_k_offset, vmm_k_offset, vmm_tmp.s);
        }
            ...
            if (jpp.ind_dt == data_type::u8) {
                if (is_tail_processing(bci)) {
                    if (jpp.is_c_padded) {
                        ...
                        zip1(p_tmp0.b, k_c_tail_mask.b, p_all_zero.b);
                        zip1(p_tmp0.h, p_tmp0.h, p_all_zero.h);
                        xa_->not_(p_tmp0.b, P_ALL_ONE / Xbyak_aarch64::T_z,
                                p_tmp0.b);
                        xa_->mov(z_tmp0.d, vr.d);
                        xa_->mov(z_tmp0.s, p_tmp0 / T_m, 0);
                        umin(z_tmp0.s, 255);
                        st1b(z_tmp0.s, p_512,
                                Xbyak_aarch64::ptr(X_DEFAULT_ADDR));
                    } else {
...
inline void jit_uni_pool_kernel<isa>::max_step_bwd(int ur_w, int ur_bc,
    ...
    for (int bci = 0; bci < ur_bc; bci++) {
        ...
        if (jpp.ind_dt == data_type::u8) {
            if (is_tail_processing(bci) && !jpp.is_c_padded) {
                add_imm(X_DEFAULT_ADDR, reg_index, step_index, X_TMP_0);
                ld1b(indvr.b, k_c_tail_mask / Xbyak_aarch64::T_z,
                        Xbyak_aarch64::ptr(X_DEFAULT_ADDR));
    ...
    if (jpp.simple_alg && jpp.ndims == 5) {
        ...
        ldr(reg_kd_pad_shift, Xbyak_aarch64::ptr(X_DEFAULT_ADDR));
        L(kd_label);
    ...
    L(kh_label);
    {
        for (int ki = 0; ki < kw; ki++) {
            ...
            for (int bci = 0; bci < ur_bc; bci++) {
                ...
                cmpeq(k_store_mask.s, p_lsb / Xbyak_aarch64::T_z, indvr,
                        vmm_k_offset);
                xa_->not_(p_tmp0.b, P_ALL_ONE.b, k_store_mask.b);
                xa_->mov(vmm_tmp.d, outvr);
                xa_->mov(vmm_tmp.s, p_tmp0 / T_m, 0);
                xa_->fadd(inpvr, inpvr, vmm_tmp.s);

                store(inpvr.getIdx(), aux_xreg_input, inp_offset,
                        is_tail_processing(bci));
            }

            xa_->add(vmm_k_offset, vmm_k_offset, vmm_one);
        }
...
void jit_uni_pool_kernel<isa>::zero_diff_src(
    ...
    ldr(reg_zero_id, Xbyak_aarch64::ptr(X_DEFAULT_ADDR));
    ...
    ldr(reg_zero_ih, Xbyak_aarch64::ptr(X_DEFAULT_ADDR));
...
void jit_uni_pool_kernel<isa>::generate() {
    ...
    ldr(reg_k_shift, Xbyak_aarch64::ptr(X_DEFAULT_ADDR));

    add_imm(X_DEFAULT_ADDR, reg_param, GET_OFF(ker_area_h), X_TMP_0);
    ldr(reg_ker_area_h, Xbyak_aarch64::ptr(X_DEFAULT_ADDR));

    add_imm(X_DEFAULT_ADDR, reg_param, GET_OFF(ur_bc), X_TMP_0);
    ldr(reg_nbc, Xbyak_aarch64::ptr(X_DEFAULT_ADDR));
