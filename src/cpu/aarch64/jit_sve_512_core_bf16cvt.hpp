...
private:
    void vcvtneps2bf16(const Xmm_t &out, const Xmm_t &in, const Xmm_t &tr0,
            const Xmm_t &one, const Xmm_t &even, const Xmm_t &selector) {
    ...
    void vcvtneps2bf16(const Xbyak_aarch64::AdrNoOfs &out, const Xmm_t &in,
            const Xmm_t &tr0, const Xbyak_aarch64::AdrNoOfs &one,
            const Xmm_t &even, const Xbyak_aarch64::AdrNoOfs &selector) {
        //host_->vpsrld(tr0, in, 16);
        CG::lsr(xa::ZRegS(IDX(tr0)), xa::ZRegS(IDX(in)), 16);
        CG::mov(xa::ZReg(IDX(tr0)).s, P_MSB_384 / xa::T_m, 0);
