...
bool jit_uni_binary_t<src_type>::post_ops_ok(
    ...
    const bool is_sve_512 = mayiuse(sve_512);
    ...
    const int vlen = cpu_isa_traits<sve_512>::vlen;
            ...
            && IMPLICATION(postops_per_oc_broadcast_exists,
                    binary_injector::all_binary_postop_rhs_per_oc_broadcast(p,
                            ...
                            [&dst_d](const memory_desc_wrapper &rhs_arg_md) {
                                return IMPLICATION(!mayiuse(sve_512),
...
struct jit_uni_binary_kernel_t : public binary_kernel_t {
    ...
    bool is_bf16_;
    const bool is_sve_512 = utils::one_of(isa, sve_512);
    ...
    const XReg &reg_offt_src0_count_ = x12;
    const XReg &reg_offt_src1_ = x7;
    ...
    const TReg treg_scales_src1_ = TReg(18);
    const TReg treg_tmp0_ = TReg(28);
    const TReg treg_tmp1_ = TReg(29);

    size_t unroll_regs_ = 8;
    size_t tail_size_ = 0;
    ...
    static constexpr cpu_isa_t inject_isa = sve_512;
    ...
    void init_post_ops_injector() {
        ...
        const eltwise_injector::static_params_t esp(true /*save_state*/,
                reg_elt_inj_table_, elt_inj_opmask_, elt_inj_tmp0_,
                elt_inj_all_, true /*is_fwd*/, false /*use_dst*/);
    ...
    void apply_postops(int unroll, bool tail) {
        ...
        for (int treg_idx = 1; treg_idx < unroll + 1; treg_idx++) {
            if (utils::one_of(op_type_, op_t::c_blocked, op_t::n_c_spatial)) {
                XReg x_addr = param1;
                if (PARAM_OFF(oc_l_off)) {
                    add_imm(X_DEFAULT_ADDR, param1, PARAM_OFF(oc_l_off),
                            X_TMP_0);
                ...
                rhs_arg_params.treg_idx_to_oc_elem_off_val.emplace(
                        treg_idx, (treg_idx - 1) * static_cast<int>(simd_w_));
            }
            if (tail) rhs_arg_params.treg_tail_idx_.emplace(treg_idx);
        }
        postops_injector_->compute_vector_range(1, unroll + 1, rhs_arg_params);
    }

    void load_kernel_params() {
        mov_imm(W_TMP_0, float2int(sum_scale_));
        dup(tsum_scale_.s, W_TMP_0);

        add_imm(X_TMP_0, reg_param_, PARAM_OFF(spat_offt_count),
                X_DEFAULT_ADDR);
        add_imm(X_TMP_1, reg_param_, PARAM_OFF(src0), X_DEFAULT_ADDR);
        add_imm(X_TMP_2, reg_param_, PARAM_OFF(src1), X_DEFAULT_ADDR);
        add_imm(X_TMP_3, reg_param_, PARAM_OFF(dst), X_DEFAULT_ADDR);
        ldr(reg_offt_src0_count_, ptr(X_TMP_0));
        ldr(reg_src0_, ptr(X_TMP_1));
        ldr(reg_src1_, ptr(X_TMP_2));
        ldr(reg_dst_, ptr(X_TMP_3));

        if (do_scale_src0_) {
            add_imm(X_DEFAULT_ADDR, reg_param_, PARAM_OFF(scales_src0),
                    X_TMP_0);
            ldr(reg_scales_src0_, ptr(X_DEFAULT_ADDR));
        }
        if (do_scale_src1_) {
            add_imm(X_DEFAULT_ADDR, reg_param_, PARAM_OFF(scales_src1),
                    X_TMP_0);
            ldr(reg_scales_src1_, ptr(X_DEFAULT_ADDR));
        }
    }

    AdrNoOfs src0_ptr(size_t offt = 0) {
        add(X_DEFAULT_ADDR, reg_src0_, reg_offt_src0_);
        if (offt) add_imm(X_DEFAULT_ADDR, X_DEFAULT_ADDR, offt, X_TMP_0);

        return ptr(X_DEFAULT_ADDR);
    }

    AdrNoOfs src1_ptr(size_t offt = 0) {
    ...
    AdrNoOfs dst_ptr(size_t offt = 0) {
        add(X_DEFAULT_ADDR, reg_dst_, reg_offt_src0_);
        if (offt) add_imm(X_DEFAULT_ADDR, X_DEFAULT_ADDR, offt, X_TMP_0);

        return ptr(X_DEFAULT_ADDR);
    }

    void perform_op(const TReg &v0, const TReg &v1, const TReg &s_src0,
            const TReg &s_src1) {
        using namespace alg_kind;
        using TRegS = cpu_isa_traits<sve_512>::TRegS;

        const TRegS t0(v0.getIdx());
        const TRegS t1(v1.getIdx());
        const TRegS t_src0(s_src0.getIdx());
        const TRegS t_src1(s_src1.getIdx());

        const auto alg = pd_->desc()->alg_kind;
        if (do_scale_src0_) fmul(t0, t0, t_src0);
        ...
        if (alg == binary_add)
            ...
            fmaxnm(t0, p_all / T_m, t1);
        else if (alg == binary_min)
            fminnm(t0, p_all / T_m, t1);
        else if (alg == binary_div)
            fdiv(t0, p_all / T_m, t1);
    ...
    void forward() {
        ...
        if (do_scale_src0_) {
            ldr(W_TMP_0, ptr(reg_scales_src0_));
            dup(treg_scales_src0_.s, W_TMP_0);
        }
        ...
        eor(reg_offt_src0_, reg_offt_src0_, reg_offt_src0_);
        // offt_src1 to get addr of src1
        eor(reg_offt_src1_, reg_offt_src1_, reg_offt_src1_);
        ...
        if (do_scale_src1_) {
            ldr(W_TMP_0, ptr(reg_scales_src1_));
            dup(treg_scales_src1_.s, W_TMP_0);
            if (broadcast_src1_value_ || offt_src1_ == 0)
                fmul(tbcast_src1_.s, tbcast_src1_.s, treg_scales_src1_.s);
        }
        ...
        L(unroll_loop);
        {
            ...
            sub_imm(reg_reverse_spat_offt_, reg_reverse_spat_offt_, offt,
                    X_TMP_0);
            add_imm(reg_offt_src0_, reg_offt_src0_, offt, X_TMP_0);
            if (use_stride_src1_)
                add_imm(reg_offt_src1_, reg_offt_src1_, offt, X_TMP_0);
            if (use_stride_rhs_postops_)
                add_imm(reg_off_rhs_postops_, reg_off_rhs_postops_, offt_elems,
                        X_TMP_0);
            b(unroll_loop);
        }
        ...
        L(unroll_loop_tail);
        {
            ...
            sub(reg_reverse_spat_offt_, reg_reverse_spat_offt_, vec_size);
            add(reg_offt_src0_, reg_offt_src0_, vec_size);
            if (use_stride_src1_) add(reg_offt_src1_, reg_offt_src1_, vec_size);
            if (use_stride_rhs_postops_)
                add(reg_off_rhs_postops_, reg_off_rhs_postops_, simd_w_);
...
template <data_type_t src_type>
struct jit_uni_binary_subkernel_t<sve_512, src_type>
    ...
    void load_no_tail(const TReg &dst, const AdrNoOfs &src, data_type_t dt) {
        switch (dt) {
            case data_type::f32: ldr(dst, src); break;
            default: assert(!"unreachable");
        }
    }

    void load_tail(const TReg &dst, const PReg &opmask, const AdrNoOfs &src,
            data_type_t dt) {
        switch (dt) {
            case data_type::f32: ld1w(dst.s, opmask / T_z, src); break;
            default: assert(!"unreachable");
        }
    }

    void store_no_tail(const AdrNoOfs &dst, const TReg &src, data_type_t dt) {
        switch (dt) {
            case data_type::f32: str(src, dst); break;
            default: assert(!"unreachable");
        }
    }

    void store_tail(const AdrNoOfs &dst, const PReg &opmask, const TReg &src,
            data_type_t dt) {
        switch (dt) {
            case data_type::f32: st1w(src.s, opmask, dst); break;
            default: assert(!"unreachable");
        }
    }

    void load(const TReg &dst, const AdrNoOfs &src, data_type_t dt, bool tail) {
        if (!tail)
    ...
    void store(
            const AdrNoOfs &dst, const TReg &src, data_type_t dt, bool tail) {
        if (!tail)
    ...
    void bcast(const TReg &dst, const AdrNoOfs &src, data_type_t dt) {
        switch (dt) {
            case data_type::f32:
                ldr(W_TMP_0, src);
                dup(dst.s, W_TMP_0);
    ...
    void compute_bcast(bool tail) override {
        if (broadcast_src1_value_)
            bcast(tbcast_src1_, src1_ptr(), src_type);
        else if (offt_src1_ == 0)
            load(tbcast_src1_, src1_ptr(), src_type, tail);
    }

    void compute_dst(int unroll, bool tail) override {
        for (int i = 0; i < unroll; i++) {
            const TReg treg_tmp_src0 = TReg(i + 1);
            const TReg treg_tmp = TReg(unroll + i + 1);
            const TReg treg_tmp_src1 = offt_src1_ ? treg_tmp : tbcast_src1_;
            load(treg_tmp_src0, src0_ptr(i * offt_src0_), src_type, tail);

            if (offt_src1_) {
                load(treg_tmp_src1, src1_ptr(i * offt_src1_), src_type, tail);
            }
            perform_op(treg_tmp_src0, treg_tmp_src1, treg_scales_src0_,
                    treg_scales_src1_);
            if (do_sum_) {
                load(treg_tmp, dst_ptr(i * offt_src0_), src_type, tail);
                fmla(treg_tmp_src0.s, p_all / T_m, treg_tmp.s, tsum_scale_.s);
            }
        }
        ...
        for (int i = 0; i < unroll; i++) {
            const TReg treg_tmp_src0 = TReg(i + 1);
            store(dst_ptr(i * offt_src0_), treg_tmp_src0, src_type, tail);
        }
    }
...
binary_kernel_t *create_binary_kernel(const binary_pd_t *pd, bool tail_kernel) {
    if (mayiuse(sve_512)) {
        using subkernel_t = jit_uni_binary_subkernel_t<sve_512, src_type>;
